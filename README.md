...
## 🏗️ Como Compilar e Gerar o Aplicativo Móvel (APK / AAB)

Caso queira gerar uma versão autônoma (standalone) para instalar diretamente em aparelhos Android (`.apk` de teste) ou para publicação na Google Play Store (`.aab`), o projeto utiliza a ferramenta **EAS (Expo Application Services)** em nuvem, eliminando a necessidade de configurar ambientes Android SDK locais.

1. **Instalar EAS CLI globalmente**:
   ```bash
   npm install -g eas-cli
   ```
2. **Efetuar login na conta Expo**:
   ```bash
   eas login
   ```
   _(Crie uma conta gratuita em [expo.dev](https://expo.dev) caso não possua)._
3. **Configurar o EAS no projeto (apenas na primeira vez)**:
   ```bash
   eas build:configure
   ```
4. **Compilar para Android (APK de teste)**:
   ```bash
   eas build --platform android --profile preview
   ```
   _(Aguarde a conclusão do build na nuvem e baixe o `.apk` gerado pelo link fornecido no final)._
5. **Compilar para Android (Produção AAB)**:
   ```bash
